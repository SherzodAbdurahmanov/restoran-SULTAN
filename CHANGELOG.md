...
### 2026-04-06: Исправлена проблема с кешем меню ✅
**Проблема**: После изменения доступности блюда в админке, в меню продолжало отображаться старое состояние

**Решение**:
1. Создана утилита `menuCache` для централизованного управления кешем
2. Автоматическая очистка кеша при изменении доступности блюда в админке
3. Посетители теперь видят актуальное состояние блюд после обновления страницы

Подробнее: смотрите `CACHE_FIX.md`

### 2026-04-06: Полная оптимизация производительности ✅
**Проблема**: Изображения загружались очень медленно (1-3 МБ каждое)

**Решение**:
1. Автоматическое сжатие изображений на 70% (145 МБ → 44 МБ)
2. Прогрессивная загрузка с красивыми placeholder
3. Кеширование данных меню (повторная загрузка мгновенная)
4. Code splitting и preload критических ресурсов
5. Оптимизация слайдера на главной странице

Подробнее: смотрите `OPTIMIZATION.md`
